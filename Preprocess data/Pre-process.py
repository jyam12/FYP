...
def generate_next_mark_price():
    ...
    Processed_Dataset["next_mark_price"] = Processed_Dataset["mark_price"].shift(-1)  # Predict next mark_price (shift by -1)
...
def generate_actual_label():
    Processed_Dataset['Price Movement'] = Processed_Dataset.apply(lambda row: 'Up' if row['next_mark_price'] > row['mark_price'] else 'Down' if row['next_mark_price'] < row['mark_price'] else 'Stable', axis=1)

generate_actual_label()

def gerenrate_actual_direction():
    Processed_Dataset["actual_direction"] = np.sign(Processed_Dataset["next_mark_price"] - Processed_Dataset["mark_price"])

gerenrate_actual_direction()

Processed_Dataset.to_csv('./datasets/PreProcessed_Dataset.csv', index=False)
